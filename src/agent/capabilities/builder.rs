...
use super::{
    CapabilityEnhancedAgent, CapabilityManager, CodeExecutionCapability, ReActCapability,
    ReflectionCapability, TaskPlanningCapability,
};
...
impl<A: Agent> CapabilityAgentBuilder<A> {
    ...
    pub fn with_reflection_priority<R: ReflectionCapability + 'static>(
        ...
        self.capabilities
            .add_capability_with_priority(capability, priority);
        self
    }

    /// Add a task planning capability
    pub fn with_task_planning<T: TaskPlanningCapability + 'static>(
        mut self,
        capability: T,
    ) -> Self {
        self.capabilities.add_capability(capability);
    ...
    pub fn with_code_execution<C: CodeExecutionCapability + 'static>(
        mut self,
        capability: C,
    ) -> Self {
        self.capabilities.add_capability(capability);
    ...
    pub async fn build(mut self) -> Result<CapabilityEnhancedAgent<A>, AgentError> {
        ...
        if let Some(config) = self.initialization_config {
            enhanced
                .capabilities
                .initialize_capabilities(config)
                .await?;
        }
...
mod tests {
    ...
    // Import the CapableAgent trait to access capabilities() method
    use super::super::CapableAgent;
