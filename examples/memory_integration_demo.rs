...
use langchain_rust::{
    agent::{
        AgentExecutor, ConversationalAgentBuilder, HumanAgentBuilder, InterventionCondition,
        TeamAgentBuilder, TeamHumanAgentBuilder, TerminationCondition,
    },
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let team_agent = TeamAgentBuilder::sequential_team([
        (
            "math_agent",
            math_agent.clone() as Arc<dyn langchain_rust::agent::Agent>,
        ),
        (
            "analysis_agent",
            analysis_agent.clone() as Arc<dyn langchain_rust::agent::Agent>,
        ),
    ])
    ...
    let _human_agent = HumanAgentBuilder::keyword_intervention(vec!["help", "review"])
        .max_interventions(3)
        .input_timeout(30)
    ...
    let _hybrid_agent = TeamHumanAgentBuilder::new()
    ...
    let _executor = AgentExecutor::from_agent(team_agent).with_memory(team_memory.clone());
